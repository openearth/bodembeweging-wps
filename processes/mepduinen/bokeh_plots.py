...
class bokeh_Plot:
    ...
    def plot_Tseries(self):
        ...
        p = figure(width=800, height=540, x_axis_type="datetime", title='Locatie id = {}'.format(self.properties), tools=TOOLS)
        p.xaxis.axis_label = 'Datum'
                
        #p.line(t,v, color="red")
        p.circle(x=self.datax, y=self.datay, size=3, color="red")
        p.y_range = Range1d(0, max(self.datay)*1.5)
