...
class LS:
    ...
    def intersect(self, all_box=False):
        ...
        self.coords = np.array(list(zip(self.ylist,self.xlist))).T
    ...
    def json(self):
        ...
        self.distances = list(map(float, np.linspace(0,len(self.values),len(self.values)) * (self.length / len(self.values))))
        return json.dumps(list(zip(self.distances,self.values)))
