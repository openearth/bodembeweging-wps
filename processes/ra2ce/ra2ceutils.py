...
def calccosts(cf, layer_name, json_matrix):
    ...
    engine = create_engine('postgresql+psycopg2://' + cf.get('PostGis', 'user')
                           + ':' + cf.get('PostGis', 'pass') + '@' + cf.get('PostGis', 'host') + ':' + str(
        cf.get('PostGis', 'port'))
    ...
    for societalIndex in range(len(values)):
        for repairIndex in range(len(values[societalIndex])):
            strSql = """update temp.{new_layer} set visclass = {val}
                            where societal_class = {s} and repair_class = {r};""".format(new_layer=new_layer,
                                                                                        val=values[societalIndex][
                                                                                            repairIndex],
                                                                                        s=societalIndex + 1,
                                                                                        r=repairIndex + 1)
            ressql = engine.execute(strSql)
    ...
    cat = Catalog("{}/rest/".format(cf.get('GeoServer', 'host')), username=cf.get('GeoServer', 'user'),
                  password=cf.get('GeoServer', 'pass'))
    datastore = cat.get_store('temp')
    ft = cat.publish_featuretype(new_layer, datastore, native_crs ='EPSG:28992', jdbc_virtual_table = '')
    cat.save(ft)
    layer = cat.get_layer(new_layer)

    # check if shapefile is line or point
    if layer_name in ["bruggen", "opdrijvenlichtgewichtconstructies", "opdrijventunnels"]:
        style = "ra2ce_punt"
    else:
        style = "ra2ce"
...
def readConfig():
    ...
    configf=join(dirname(realpath(__file__)), '../../config/ra2ce.txt')
