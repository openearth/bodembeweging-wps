...
def readConfig():
    ...
    cfile=os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../config/nutrientenmaas.txt')
...
def check_location(location, epsgin='epsg:3857'):
    ...
    try:
        ...
        logger.info('''Input Coordinates {} {} -> {} {}'''.format(xin,yin,lon,lat))
...
def queryPostGISClosestPoint(xk, yk, epsg=28992):
    # DB connect
    cf = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../config/nutrientenmaas.txt')
    # credentials = sqlfunctions.get_credentials(cf)
    credentials = sqlfunctions.pgconfig(cf, section = 'postgres')
...
def queryPostGISClosestPointNitrate(xk, yk, epsg=28992):
    ...
    cf = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../config/nutrientenmaas.txt')
    # credentials = sqlfunctions.get_credentials(cf)
    credentials = sqlfunctions.pgconfig(cf, section = 'nitrate')
    # Look for closes transect ()
    sql = """SELECT *,
