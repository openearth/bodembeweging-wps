...
def sql_engine(path):
        ...
        path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../config/nutrientenmaas.txt') #requires config file with section header
        cf = readconf(path)
...
class Pie_plot:
        ...
        def to_dict(self):
                ...
                for row in self.input_data:
                        dic_n = dict((key, value) for key, value in list(row.items())) #sqlalchemy object to dict

                #dict with fields only for the plotting
                sub = ['act_bem_{}','his_bem_{}', 'nlev_bod_{}','ua_nat_{}','ua_kwel_{}','landb_ov_{}','rwzi_{}','overst_{}','antrop_{}','dep_op_{}','buitenl_{}','rijkswat_{}','rwzi_afw_{}','afw_ov_{}','nat_bovenstr','landb_bovenstr']
                fields = []
                for i in sub:
