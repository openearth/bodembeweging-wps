...
# $Id: opendap_nhi.py 15854 2019-10-21 11:25:19Z pronk_mn $
# $Date: 2019-10-21 13:25:19 +0200 (ma, 21 okt 2019) $
# $Author: pronk_mn $
# $Revision: 15854 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/nhidataportaal/opendap_nhi.py $
# $Keywords: $
...
def nhi_spreidingslengte_XYL(x, y, layer):
    # Urls Opendap
    url_vert_res = '''http://opendap-nhi-data.deltares.nl/thredds/dodsC/opendap/Modellen/NHI/modelinvoer/verical_resistance/vertical_resistance_layer{}.nc'''.format(
        layer)
    url_transm = '''http://opendap-nhi-data.deltares.nl/thredds/dodsC/opendap/Modellen/NHI/modelinvoer/transmissivity/transmissivity_layer{}.nc'''.format(
        layer)
...
def find_spreidingslengte(x, y):
    for l in range(1, 7):
        ...
        slm = np.mean(sl1)
        logging.info(
            'layer=L{} - Spreidingslengte_3x3_mean: (x={}, y={}) => (Kd={}, c={}, sl={})'.format(l, x, y, Kdm, cm, slm))
...
def nhi_invoer(x, y, layers=list(range(1, 7))):
